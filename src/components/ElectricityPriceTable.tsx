...
export function ElectricityPriceTable({ todayPoints, tomorrowPoints, surcharge }: { todayPoints: PricePoint[]; tomorrowPoints: PricePoint[]; surcharge: Surcharge }) {
  ...
  const rows = useMemo(() => {
    ...
    if (sort === "price") withTotals.sort((a, b) => a.price - b.price);
    return withTotals;
          ...
          <table className="w-full border-collapse text-sm">
            <thead className="sticky top-0 bg-white dark:bg-neutral-900">
              <tr className="text-left text-xs uppercase tracking-wide text-black/50 dark:text-white/50">
                ...
                <th className="px-3 py-2 text-right font-medium">Prijs / kWh</th>
              </tr>
            </thead>
            <tbody>
              {rows.map(({ point, price }) => {
                ...
                return (
                  <tr
                    key={point.timestamp}
                    className={`border-t border-black/5 dark:border-white/5 ${isNow ? "bg-blue-50 dark:bg-blue-500/10" : isCheapest ? "bg-emerald-50 dark:bg-emerald-500/10" : isExpensive ? "bg-red-50 dark:bg-red-500/10" : ""}`}
                  >
                    ...
                    <td className={`px-3 py-1.5 text-right tabular-nums font-medium ${isCheapest ? "text-emerald-600 dark:text-emerald-400" : isExpensive ? "text-red-600 dark:text-red-400" : ""}`}>{formatEurCents(price)}</td>
                  </tr>
                );
              })}
            </tbody>
